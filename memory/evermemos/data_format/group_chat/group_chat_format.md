...
- **private**: Human-human private chat (one-on-one)
- **group**: Group chat with multiple participants

**Private Scene Example**:
```json
"conversation_meta": {
  "scene": "private",
  "scene_desc": {
...
- **scene**: Scene type identifier (`private` or `group`)
- **scene_desc**: Scene description information
  - `owner_user_id` identifies the owner/self user for profile extraction
  - `owner_user_name` is optional display name for the owner
