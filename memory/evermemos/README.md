...
- Loads `data/assistant_chat_zh.json`, appends `scene="private"` to each message, and streams every entry to `http://localhost:8001/api/v1/memories`. Update the `base_url`, `data_file`, or `profile_scene` constants in `demo/extract_memory.py` if you host the API on another endpoint or want to ingest a different scenario.
...
> ℹ️ `scene` is a required field, only supports `private` or `group`, used to specify memory extraction strategy.
...
> - Use `private` for one-on-one private chats
> - Use `group` for multi-person group discussions
> 
> **Note**: `scene` only accepts `private` or `group` in the current pipeline.
