...
**Features**:
- Interactive language selection (Chinese/English) and scenario selection (Private/Group Chat)
...
2. **Scenario**: Choose `[1] Private Mode` or `[2] Group Chat Mode`
...
|----------|----------|----------|
| Private Chat | Chinese | `data/assistant_chat_zh.json` |
| Private Chat | English | `data/assistant_chat_en.json` |
...
### Private Scenario (assistant_chat_en.json / assistant_chat_zh.json)

**Conversation Context:** One-on-one private chat

**Key Contents:**
- Daily life updates and preferences
- Plans and reminders
...
**Private Chat Scenario Examples:**
...
- `profile_scene`: Set to `"private"` or `"group"`
...
- **Private mode (private)**: One-on-one conversations, extract personalized profiles
- **Group chat mode (group)**: Multi-participant conversations, extract group memories and member profiles
