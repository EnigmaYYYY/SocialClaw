...
class ExtractModeConfig:
    ...
    scenario_type: ScenarioType = ScenarioType.GROUP
    language: str = "zh"
    ...
    def __post_init__(self):
        ...
        if self.scenario_type == ScenarioType.GROUP:
            self.group_id = self.group_id or "group_chat_001"
            self.group_name = self.group_name or "Group Chat"
            self.enable_foresight_extraction = False
        else:
            self.group_id = self.group_id or "private_chat_001"
            self.group_name = self.group_name or "Private Chat"
