    ...
    def __post_init__(self):
        ...
        if not self.base_url:
            ...
            if self.provider == RerankProvider.DEEPINFRA:
                default_url = "https://api.deepinfra.com/v1/inference"
            elif self.provider == RerankProvider.SILICONFLOW:
                default_url = "https://api.siliconflow.cn/v1/rerank"
            else:
                default_url = "http://localhost:12000/score"
            self.base_url = os.getenv("RERANK_BASE_URL", default_url)

        if not self.model:
            default_model = (
                "BAAI/bge-reranker-v2-m3"
                if self.provider == RerankProvider.SILICONFLOW
                else "Qwen/Qwen3-Reranker-4B"
            )
            self.model = os.getenv("RERANK_MODEL", default_model)
...
class RerankService(RerankServiceInterface):
    ...
    async def _send_rerank_request_batch(
        ...
        if self.config.provider == RerankProvider.SILICONFLOW:
            request_data = {
    ...
    def _parse_provider_response(self, json_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif self.config.provider == RerankProvider.SILICONFLOW:
            if "results" in json_body:
                results = json_body["results"]
