...
class FetchMemoryServiceImpl(FetchMemoryServiceInterface):
    ...
    def __init__(self):
        ...
        self._unified_profile_repo = None
    ...
    def _get_repositories(self):
        ...
        if self._unified_profile_repo is None:
            self._unified_profile_repo = get_bean_by_type(UnifiedProfileRepository)
    ...
    async def _convert_core_memory(self, core_memory) -> CoreMemoryModel:
        ...
        return CoreMemoryModel(
            ...
            user_to_friend_catchphrase=getattr(
                core_memory, "user_to_friend_catchphrase", None
            ),
    ...
    def _convert_event_log(
        ...
        return EventLogModel(
            ...
            conversation_id=getattr(event_log, 'conversation_id', None) or event_log.group_id,
    ...
    def _convert_foresight_record(
        ...
        return ForesightRecordModel(
            ...
            conversation_id=getattr(foresight_record, 'conversation_id', None) or foresight_record.group_id,
        ...
        try:
            ...
            match memory_type:
                ...
                case MemoryType.BASE_MEMORY:
                    ...
                    if core_memory:
                        ...
                        BaseMemoryModel(
                            ...
                            user_id=core_memory.user_id,
                            content=(
                                f"User: {base_info.get('user_name', 'Unknown')} | "
                                f"Gender: {base_info.get('gender', 'Unknown')} | "
                                f"Location: {base_info.get('base_location', 'Unknown')}"
                            ),
                            created_at=core_memory.created_at,
                            updated_at=core_memory.updated_at,
                            metadata={
                                ...
                                "base_location": base_info.get("base_location", ""),
                                "age": base_info.get("age", None),
                            },
                        )
                    ]
                    else:
                ...
                case MemoryType.PROFILE:
                    # Profile: get user profiles from unified_profile_repo (single source of truth)
                    unified_profiles = await self._unified_profile_repo.list_contact_profiles(user_id)
                    # Also include the user's own profile
                    user_own_profile = await self._unified_profile_repo.get_user_profile(user_id)

                    memories = []
                    all_profiles = list(unified_profiles or [])
                    if user_own_profile:
                        all_profiles.insert(0, user_own_profile)

                    for up in all_profiles[:limit]:
                        memories.append({
                            "id": str(up.profile_id) if up.profile_id else str(up.id) if hasattr(up, 'id') else "",
                            "user_id": up.owner_user_id,
                            "target_user_id": up.target_user_id,
                            "profile_type": up.profile_type.value if up.profile_type else None,
                            "display_name": up.display_name,
                            "facts": [f.to_dict() if hasattr(f, 'to_dict') else f for f in (up.facts or [])],
                            "social_attributes": up.social_attributes.to_dict() if up.social_attributes and hasattr(up.social_attributes, 'to_dict') else up.social_attributes,
                            "communication_style": up.communication_style.to_dict() if up.communication_style and hasattr(up.communication_style, 'to_dict') else up.communication_style,
                            "risk_assessment": up.risk_assessment.to_dict() if up.risk_assessment and hasattr(up.risk_assessment, 'to_dict') else up.risk_assessment,
                            "conversation_id": up.conversation_id,
                            "version": up.metadata.version if up.metadata else None,
