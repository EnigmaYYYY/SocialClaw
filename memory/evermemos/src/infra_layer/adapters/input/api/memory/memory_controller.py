...
class MemoryController(BaseController):
    ...
    async def memorize_single_message(
        ...
        Receive simple direct single message format (or batch messages), convert and store via group_chat_converter
        ...
        memorize_input: Dict[str, Any] | None = None
        try:
            ...
            if "messages" in message_data:
                logger.info(
                    "Detected batch messages, using provided internal format directly"
                )
                memorize_input = message_data
            elif (
                "conversation_meta" in message_data
                and "conversation_list" in message_data
            ):
                logger.info(
                    "Detected GroupChatFormat input, converting to internal format"
                )
            ...
            conversation_id = memorize_input.get("conversation_id") or memorize_input.get(
                "group_id"
            )
            if conversation_id and not memorize_input.get("group_id"):
                memorize_input["group_id"] = conversation_id
            if conversation_id and not memorize_input.get("conversation_id"):
                memorize_input["conversation_id"] = conversation_id

            logger.info(
                "Conversion completed: conversation_id=%s, group_name=%s",
                conversation_id,
                group_name,
            )
            ...
            if message_data is not None:
                logger.error(
                    "memorize request body preview: %s", summarize_json(message_data)
                )
            if memorize_input is not None:
                logger.error(
                    "memorize converted input preview: %s",
                    summarize_json(memorize_input),
                )
            ...
            logger.exception("memorize request processing failed: %s", e)
            if message_data is not None:
                logger.error(
                    "memorize request body preview: %s", summarize_json(message_data)
                )
    ...
    async def fetch_memories(self, fastapi_request: FastAPIRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if params is not None:
                logger.error("Fetch request params preview: %s", summarize_json(params))
            raise HTTPException(status_code=400, detail=str(e)) from e
        except HTTPException:
            # Re-raise HTTPException
            raise
        except Exception as e:
            logger.exception("Fetch request processing failed: %s", e)
            if params is not None:
                logger.error("Fetch request params preview: %s", summarize_json(params))
            raise HTTPException(
                status_code=500,
    ...
    async def search_memories(self, fastapi_request: FastAPIRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            conversation_id = query_params.get("conversation_id") or query_params.get(
                "group_id"
            )
            logger.info(
            ...
            logger.info(
                "💌💌💌 Chat message received: group_id=%s, message=%s",
                conversation_id,
                query,
            )
            ...
            if query_params is not None:
                logger.error(
                    "Search request params preview: %s",
                    summarize_json(query_params),
                )
            raise HTTPException(status_code=400, detail=str(e)) from e
        except HTTPException:
            # Re-raise HTTPException
            ...
            logger.exception("Search request processing failed: %s", e)
            if query_params is not None:
                logger.error(
                    "Search request params preview: %s",
                    summarize_json(query_params),
    ...
    async def save_conversation_meta(
        ...
        try:
            ...
            conversation_id = request_data.get("conversation_id") or request_data.get(
                "group_id"
            )
            if not conversation_id:
                raise HTTPException(
                    status_code=400,
                    detail="Missing required field conversation_id/group_id",
                )
            logger.info(
                "Received conversation-meta save request: conversation_id=%s",
                conversation_id,
            )
            ...
            conversation_meta = ConversationMeta(
                ...
                conversation_id=conversation_meta_request.conversation_id
                or conversation_meta_request.group_id,
            ...
            saved_meta = await self.conversation_meta_repository.upsert_by_conversation_id(
                conversation_id=conversation_meta.conversation_id
                or conversation_meta.group_id,
                conversation_data={
            ...
            logger.info(
                "Conversation metadata saved successfully: id=%s, conversation_id=%s",
                saved_meta.id,
                saved_meta.conversation_id or saved_meta.group_id,
            )
            ...
            return {
                ...
                "result": {
                    ...
                    "conversation_id": saved_meta.conversation_id
                    or saved_meta.group_id,
            ...
            logger.exception("conversation-meta request processing failed: %s", e)
            if request_data is not None:
                logger.error(
                    "conversation-meta request preview: %s",
                    summarize_json(request_data),
                )
            raise HTTPException(
                status_code=500,
    ...
    async def patch_conversation_meta(
        ...
        try:
            # 1. Get JSON body from request
            request_data = await fastapi_request.json()
            group_id = request_data.get("conversation_id") or request_data.get("group_id")
            ...
            logger.info(
                "Received conversation-meta partial update request: conversation_id=%s",
                group_id,
            )

            # 3. Check if conversation metadata exists
            existing_meta = await self.conversation_meta_repository.get_by_conversation_id(
                group_id
            )
            ...
            for key, value in request_data.items():
                if key in immutable_fields or key in {"group_id", "conversation_id"}:
                    continue
            ...
            if not update_data:
                ...
                return {
                    ...
                    "result": {
                        ...
                        "conversation_id": existing_meta.conversation_id
                        or existing_meta.group_id,
            ...
            updated_meta = await self.conversation_meta_repository.update_by_conversation_id(
                conversation_id=group_id, update_data=update_data
            )
            ...
            logger.info(
                "Conversation metadata updated successfully: id=%s, conversation_id=%s, updated_fields=%s",
                updated_meta.id,
                updated_meta.conversation_id or updated_meta.group_id,
                updated_fields,
            )
            ...
            return {
                ...
                "result": {
                    ...
                    "conversation_id": updated_meta.conversation_id
                    or updated_meta.group_id,
