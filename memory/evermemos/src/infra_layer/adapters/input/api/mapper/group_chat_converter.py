...
def convert_group_chat_format_to_memorize_input(
    ...
    # Extract unified conversation identifier (fallback to legacy group_id)
    conversation_id = conversation_meta.get("conversation_id") or conversation_meta.get(
        "group_id"
    )
    ...
    if group_id:
        ...
        result["conversation_id"] = group_id
    if group_name:
...
def convert_simple_message_to_memorize_input(
    ...
    """
    # Extract unified conversation identifier (fallback to legacy group_id)
    conversation_id = message_data.get("conversation_id") or message_data.get("group_id")
    group_id = conversation_id
