...
class EpisodicMemoryRawRepository(BaseRepository[EpisodicMemory]):
    ...
    async def get_by_conversation_id(
        ...
        """Retrieve episodic memories by conversation_id (primary key)."""
        try:
            or_conditions = [{"conversation_id": conversation_id}, {"group_id": conversation_id}]
            filter_dict: Dict[str, Any] = {"$or": or_conditions}
            if user_id:
            ...
            results = await query.to_list()
            logger.debug(
                "✅ Retrieved episodic memories by conversation_id: %s, found %d records",
                conversation_id,
                len(results),
            )
            return results
        except Exception as e:
            logger.error("❌ Failed to retrieve episodic memories by conversation_id: %s", e)
            return []

    async def delete_by_conversation_id(
        self, conversation_id: str, session: Optional[AsyncClientSession] = None
    ) -> int:
        """Delete all episodic memories by conversation_id."""
        try:
            result = await self.model.find(
                {"$or": [{"conversation_id": conversation_id}, {"group_id": conversation_id}]},
                session=session,
            ).delete()
            count = result.deleted_count if result else 0
            logger.info(
                "✅ Deleted episodic memories by conversation_id: %s, deleted %d records",
                conversation_id,
                count,
            )
            return count
        except Exception as e:
            logger.error("❌ Failed to delete episodic memories by conversation_id: %s", e)
            return 0
