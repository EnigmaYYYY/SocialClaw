...
class MemCellRawRepository(BaseRepository[MemCell]):
    ...
    async def delete_by_group_id(
        ...
        """
        Delete all MemCell of a conversation/group.

        Args:
            group_id: Group ID / conversation ID
            session: Optional MongoDB session, for transaction support
        ...
        """
        try:
            result = await self.model.find({"group_id": group_id}).delete(session=session)
            count = result.deleted_count if result else 0
            logger.info(
                "é‰?Successfully deleted all MemCell of group: %s, deleted %d records",
                group_id,
                count,
            )
            return count
        except Exception as e:
            logger.error("é‰‚?Failed to delete all MemCell of group: %s", e)
            return 0
