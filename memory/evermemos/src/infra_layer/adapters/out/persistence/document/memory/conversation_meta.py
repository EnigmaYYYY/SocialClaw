...
class ConversationMeta(DocumentBase, AuditBase):
    ...
    conversation_id: Indexed(str) = Field(
        ..., description="Unified conversation ID, used to identify a conversation"
    )
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "example": {
                ...
                "conversation_id": "chat_user_001_assistant",
    ...
    class Settings:
        ...
        indexes = [
            # conversation_id index (high-frequency query)
            IndexModel(
            ...
            # Composite index: conversation_id + scene (common compound query)
            IndexModel(
                [("conversation_id", ASCENDING), ("scene", ASCENDING)],
                name="idx_conversation_id_scene",
            ),
            # Legacy compatibility index
            IndexModel([("group_id", ASCENDING)], name="idx_group_id_legacy", sparse=True),
        ]
