...
def convert_dict_to_retrieve_mem_request(
    ...
    try:
        ...
        return RetrieveMemRequest(
            ...
            conversation_id=conversation_id,
            group_id=group_id,  # Unified conversation key (legacy field name group_id)
...
def _create_memorize_request(
    ...
    resolved_conversation_id = conversation_id or group_id
    resolved_group_id = group_id or conversation_id
...
async def handle_conversation_format(data: Dict[str, Any]) -> MemorizeRequest:
    ...
    conversation_id = data.get(DataFields.CONVERSATION_ID)
    group_id = data.get(DataFields.GROUP_ID) or conversation_id
    ...
    return _create_memorize_request(
        ...
        group_name=data.get("group_name"),
        current_time=current_time,
        owner_user_id=data.get(DataFields.OWNER_USER_ID),
    )
