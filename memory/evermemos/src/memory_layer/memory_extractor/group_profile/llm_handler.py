...
class GroupProfileLLMHandler:
    ...
    def _parse_content_response(
        ...
        try:
            ...
            if response:
                logger.error(
                    "[ContentParse] Response preview: %s",
                    summarize_text(response, 800),
                )
            if prompt:
                logger.error(
                    "[ContentParse] Prompt preview: %s",
            ...
            if artifact_path:
                logger.error(
                    "[ContentParse] LLM artifact saved: %s", artifact_path
                )
    ...
    def _parse_behavior_response(
        ...
        try:
            ...
            if response:
                logger.error(
                    "[BehaviorParse] Response preview: %s",
                    summarize_text(response, 800),
                )
            if prompt:
                logger.error(
                    "[BehaviorParse] Prompt preview: %s",
                    summarize_text(prompt, 800),
                )
            artifact_path = dump_llm_artifacts(
                "group_profile_behavior",
                prompt=prompt,
                response=response,
                meta={"error": str(e)},
            )
            if artifact_path:
                logger.error(
                    "[BehaviorParse] LLM artifact saved: %s", artifact_path
                )
