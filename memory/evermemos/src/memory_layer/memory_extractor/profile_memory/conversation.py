...
def _normalize_speaker_name(speaker_id: Any, speaker_name: Any, user_id_to_name: Dict[str, str]) -> str:
    speaker_id_str = str(speaker_id).strip() if speaker_id else ""
    speaker_name_str = str(speaker_name).strip() if speaker_name else ""
    if not speaker_name_str and speaker_id_str:
        speaker_name_str = user_id_to_name.get(speaker_id_str, "")
    return speaker_name_str or speaker_id_str or "Unknown"


def _build_participant_header_from_data_list(
    data_list: List[Any], user_id_to_name: Dict[str, str]
) -> str:
    seen: List[str] = []
    ...
    for speaker_id in seen:
        speaker_name = user_id_to_name.get(speaker_id, "") or speaker_id
        participants.append(f"{speaker_name}(user_id:{speaker_id})")
    return "participants: " + ", ".join(participants) if participants else ""


def _build_participant_header_from_messages(
    messages: List[Dict[str, Any]], user_id_to_name: Dict[str, str]
) -> str:
    seen: List[str] = []
...
def build_conversation_text(
    ...
    """
    # Use group_id as conversation_id for consistency with Redis cache
    ...
    if conversation_id_str:
        lines.append(f"conversation_id: {conversation_id_str}")
    participant_header = _build_participant_header_from_data_list(data_list, user_id_to_name)
    if participant_header:
        lines.append(participant_header)
    for data in data_list:
        speaker = _normalize_speaker_name(
            data.get("speaker_id", ""),
...
def build_owner_conversation_lines(
    memcell: MemCell, user_id_to_name: Dict[str, str], owner_user_id: Optional[str]
) -> List[str]:
    """Build conversation lines for messages sent by the owner."""
    ...
    for data in data_list:
        speaker_id = data.get("speaker_id", "")
        if str(speaker_id) != owner_user_id:
            continue
...
def build_raw_message_lines(
    memcell: MemCell, user_id_to_name: Dict[str, str]
) -> List[str]:
    """Build minimally processed raw message lines for LLM grounding."""
    # Use group_id as conversation_id for consistency with Redis cache
    conversation_id = getattr(memcell, "group_id", None) or getattr(memcell, "event_id", None)
    conversation_id_str = str(conversation_id) if conversation_id is not None else ""
    data_list: List[Any] = getattr(memcell, "original_data", []) or []
    ...
    for data in data_list:
        if not isinstance(data, dict):
            continue
        speaker_name = _normalize_speaker_name(
            data.get("speaker_id", ""),
            data.get("speaker_name", ""),
            user_id_to_name,
        )
        ...
        if isinstance(content, str):
            content = append_refer_user_ids(content, refer_list)
        if timestamp:
            lines.append(f"[{timestamp}] {speaker_name}: {content}")
        else:
            lines.append(f"{speaker_name}: {content}")
    return lines


def build_raw_message_lines_from_messages(
    messages: List[Dict[str, Any]], user_id_to_name: Dict[str, str]
) -> List[str]:
    """Build minimally processed raw message lines directly from message list."""
    lines: List[str] = []
    if messages:
        first_conversation_id = str(messages[0].get("conversation_id", "")).strip()
        if first_conversation_id:
            lines.append(f"conversation_id: {first_conversation_id}")
        participant_header = _build_participant_header_from_messages(messages, user_id_to_name)
        if participant_header:
            lines.append(participant_header)
    for data in messages:
        if not isinstance(data, dict):
...
def build_conversation_text_from_messages(
    messages: List[Dict[str, Any]], user_id_to_name: Dict[str, str]
) -> str:
    """Convert raw messages into formatted conversation text."""
    lines: List[str] = []
    if messages:
        first_conversation_id = str(messages[0].get("conversation_id", "")).strip()
        if first_conversation_id:
...
def build_owner_conversation_lines_from_messages(
    messages: List[Dict[str, Any]],
    user_id_to_name: Dict[str, str],
    owner_user_id: str
) -> List[str]:
    """Build conversation lines for messages sent by the owner from raw messages."""
    if not owner_user_id:
        return []
    ...
    for data in messages:
        if isinstance(data, dict):
            first_conversation_id = str(data.get("conversation_id", "")).strip()
            if first_conversation_id:
                break
    if first_conversation_id:
        lines.append(f"conversation_id: {first_conversation_id}")
...
def build_profile_prompt(
    ...
    participants_current_profiles: List[Dict[str, Any]],
    ...
    participants_payload = json.dumps(participants or [], ensure_ascii=False)
    return (
        prompt_template.replace("{conversation}", "\n".join(conversation_lines))
        .replace("{owner_user_id}", request.owner_user_id or "")
        .replace("{participants}", participants_payload)
        .replace(
            "{participants_profile}",
            json.dumps(participants_current_profiles, ensure_ascii=False),
        )
        .replace(
...
def compute_catchphrase_candidates(
    messages: List[Dict[str, Any]],
    top_n: int = 15,
    min_count: int = 2,
) -> Dict[str, List[Tuple[str, int]]]:
    """Pre-compute high-frequency short phrases per user from raw messages.

    Splits each message on punctuation/whitespace and counts short segments
    (1-8 chars). Normalizes repeated characters and merges similar phrases
    to avoid fragmentation (e.g., "哈哈"/"哈哈哈" → "哈哈").

    Args:
        messages: Raw message list with speaker_id and content fields.
        top_n: Max candidates to return per user.
        min_count: Minimum occurrence count to be considered a candidate.

    Returns:
        {user_id: [(phrase, count), ...]} sorted by count desc.
    """
    _SPLIT_PATTERN = re.compile(r'[，。！？,.!?\s、；：""''【】()（）\[\]…~～]+')

    # 通用词黑名单（无个性特征的高频词）
    _GENERIC_WORDS = {
        "好的", "嗯", "是", "哦", "啊", "呀", "吧", "呢", "哈", "嘿", "喂",
        "好", "对", "行", "可以", "没事", "谢谢", "不客气", "抱歉", "对不起",
        "ok", "OK", "yes", "no", "yeah", "yep", "nope", "thanks", "sorry",
        "的", "了", "吗", "呢", "吧", "啊", "呀", "哦", "嗯", "唉"
    }

    def _normalize_repeated_chars(text: str) -> str:
        """Normalize repeated characters: '哈哈哈哈' → '哈哈', 'hahaha' → 'haha'."""
        if len(text) <= 2:
            return text
        ...
        # 检测是否为重复模式（如 "哈哈哈哈" 或 "hahaha"）
        for unit_len in range(1, len(text) // 2 + 1):
            unit = text[:unit_len]
            if text == unit * (len(text) // unit_len):
                # 找到重复单元，保留2个单元
                return unit * min(2, len(text) // unit_len)
    ...
    for msg in messages:
        ...
        if not speaker_id or not isinstance(content, str) or not content.strip():
            continue

        counter = user_counters.setdefault(speaker_id, Counter())
        for seg in _SPLIT_PATTERN.split(content):
            seg = seg.strip()
            if 1 <= len(seg) <= 8:
                # 归一化重复字符
                normalized = _normalize_repeated_chars(seg)
                counter[normalized] += 1

    # 第二阶段：合并相似短语并过滤通用词
    result: Dict[str, List[Tuple[str, int]]] = {}
    for user_id, counter in user_counters.items():
        # 按频次排序
        sorted_phrases = counter.most_common()
        ...
        for phrase, count in sorted_phrases:
            if phrase in processed or phrase in _GENERIC_WORDS:
                continue

            # 将当前短语作为代表
            merged_counter[phrase] = count
            processed.add(phrase)

            # 查找以当前短语为前缀的其他短语，合并计数
            for other_phrase, other_count in sorted_phrases:
                if other_phrase in processed:
                    continue
                # 如果 other_phrase 是 phrase 的扩展（如 "其实" vs "其实吧"）
                if other_phrase.startswith(phrase) and len(other_phrase) <= len(phrase) + 2:
                    merged_counter[phrase] += other_count
                    processed.add(other_phrase)
                # 如果 phrase 是 other_phrase 的扩展，用更短的作为代表
                elif phrase.startswith(other_phrase) and len(phrase) <= len(other_phrase) + 2:
                    merged_counter[other_phrase] = merged_counter.pop(phrase) + other_count
                    processed.add(phrase)
                    processed.add(other_phrase)
                    break
        ...
        candidates = [
            (phrase, count)
            for phrase, count in merged_counter.most_common(top_n * 2)
            if count >= min_count and phrase not in _GENERIC_WORDS
        ][:top_n]
