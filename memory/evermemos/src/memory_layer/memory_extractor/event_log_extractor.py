...
class EventLogExtractor:
    ...
    def _parse_llm_response(
        ...
        logger.error(
            "Unable to parse LLM response: response_len=%s response_preview=%s",
            len(response) if response else 0,
            summarize_text(response, 800),
        )
        if prompt:
            logger.error(
                "Event log prompt preview: %s",
        ...
        if artifact_path:
            logger.error("Event log parse artifact saved: %s", artifact_path)
    ...
    async def _extract_event_log(
        ...
        data = self._parse_llm_response(response, prompt=prompt)
        ...
        if artifact_path:
            logger.info("Event log parse success saved: %s", artifact_path)
