...
- 🎭 **Multi-Scenario**: Supports both private and group scenarios
...
|-----------|------|---------|-------------|
| `scenario` | `ScenarioType` or `str` | `"private"` | Extraction scenario: "private" or "group" |
...
### Group (Multi-Person Chat)

Optimized for multi-person conversations. Extracts:
- Personality and decision-making patterns
- Interests and goals
- Values, motivations, and behavioral tendencies
- Humor and colloquial style (when explicitly stated)

```python
config = ProfileManagerConfig(scenario="group")
```
...
```python
config = ProfileManagerConfig(scenario="private")
```
...
1. **Choose the right scenario**: Use "group" for multi-person chats and "private" for one-on-one chats
...
3. Verify scenario matches your use case (group vs private)
