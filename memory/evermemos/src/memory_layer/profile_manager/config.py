...
class ScenarioType(str, Enum):
    ...
    PRIVATE = "private"  # One-on-one human chat scenario
    GROUP = "group"  # Multi-person group chat scenario
...
class ProfileManagerConfig:
    ...
    Attributes:
        scenario: Profile extraction scenario (private or group)
    ...
    scenario: ScenarioType = ScenarioType.PRIVATE
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.scenario, ScenarioType):
            if isinstance(self.scenario, str):
                scene = self.scenario.lower().strip()
                # Backward compatibility: map legacy values to new scenes
                if scene in ["assistant", "private"]:
                    self.scenario = ScenarioType.PRIVATE
                elif scene in ["companion", "group", "group_chat"]:
                    self.scenario = ScenarioType.GROUP
                else:
                    self.scenario = ScenarioType(scene)
            else:
