...
列表字段（如 `occupation`、`personality`、`communication_style` 等）允许多条并列条目，但**禁止语义冗余**。在对单条现有条目执行情形 A/B/C/D 判断之前，先对整个列表执行一次**语义包含检测**：

| 关系 | 判断标准 | 示例 | 处理方式 |
|------|---------|------|---------|
| **新值更泛化**（包含已有值） | 已有值是新值的实例/子集 | 已有"学生/班长"，新观察"学生" | 情形 A：保留更具体的已有值，仅追加证据，不新增泛化条目 |
| **新值更具体**（被已有值包含） | 新值是已有值的实例/子集 | 已有"学生"，新观察"学生/班长" | 情形 B2：将已有条目精化为更具体的新值，合并证据 |
| **列表内存在互相冗余的已有条目** | 多条已有值之间存在包含关系 | 列表中同时有"学生/班长"+"学生/班级干部"+"学生" | 情形 D：合并为最具体的表述，消除父级冗余条目 |
| **新值与所有已有值并列** | 完全独立的身份/特征，无包含关系 | 已有"学生/班长"，新观察"兼职摄影师" | 正常新增独立条目 |

**语义包含判断标准**：
- 若 A 的含义成立，则 B 的含义必然也成立 → A ⊆ B（A 被 B 包含，B 更泛化）
- "学生/班长" ⊆ "学生"：班长必然是学生，反之不成立
- "学生/班长" ⊆ "学生/班级干部"：班长是班级干部的一种
- "学生/班长" 与 "学生/体育委员" **并列**：两者都是班级干部，互不包含，可共存

> **操作顺序**：新增条目前，先扫描整个列表是否存在包含关系；若有，按上表处理后停止，不再进入情形 A/B/C/D；若无任何包含关系，再正常执行情形判断。
...
│
├── 【列表字段语义预检】（仅适用于列表字段）
│   ├── 新值被某已有值包含（新值更泛化）→ 情形 A：仅追加证据，不新增条目
│   ├── 新值包含某已有值（新值更具体）→ 情形 B2：精化已有条目
│   ├── 列表内已有条目互相冗余 → 情形 D：合并冗余条目
│   └── 无包含关系 → 进入下方正常情形判断
│
├── 现有档案中是否有相关条目？
...
│
└── 输出时：本次新 evidence 追加至末尾；若总数超过 6 条，按淘汰规则精简
```
...
每个条目的 `evidences` 列表最多保留 **6 条**。追加新证据后若超过上限，按以下优先级**淘汰旧证据**，直到总数 ≤ 6：

1. **优先淘汰**：reasoning 为空或极短（<10字）的证据
2. **其次淘汰**：evidence_level 更低的证据（L2 优先于 L1 被淘汰）
3. **最后淘汰**：时间上更早的证据（event_id 字典序更小视为更早）

> 淘汰后，保留的 6 条应是最能支撑当前 value 的证据组合，不必按原始顺序排列。
"""
