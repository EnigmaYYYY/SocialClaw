...
from memory_layer.memory_extractor.group_profile_memory_extractor import GroupProfileMemory
...
def _convert_group_profile_data_to_profile_format(
    ...
    # Handle member_styles conversion: from dict/list to MemberStyle objects
    member_styles = []
    if (
        hasattr(group_profile_memory, "member_styles")
        and group_profile_memory.member_styles is not None
    ):
        for item in group_profile_memory.member_styles:
            if isinstance(item, dict):
                member_styles.append(
                    DocMemberStyle(
                        user_id=item.get("user_id", ""),
                        user_name=item.get("user_name", ""),
                        catchphrase=item.get("catchphrase") or [],
                        communication_style=item.get("communication_style") or [],
                        humor_use=item.get("humor_use") or [],
                        source=item.get("source"),
                    )
                )
            else:
                member_styles.append(item)
...
def _convert_profile_data_to_core_format(profile_memory: ProfileMemory) -> CoreMemory:
    ...
    catchphrase = None
    if hasattr(profile_memory, 'catchphrase') and profile_memory.catchphrase:
        catchphrase = profile_memory.catchphrase

    user_to_friend_catchphrase = None
    if (
        hasattr(profile_memory, 'user_to_friend_catchphrase')
        and profile_memory.user_to_friend_catchphrase
    ):
        user_to_friend_catchphrase = profile_memory.user_to_friend_catchphrase

    user_to_friend_chat_style_preference = None
    if (
        hasattr(profile_memory, 'user_to_friend_chat_style_preference')
        and profile_memory.user_to_friend_chat_style_preference
    ):
        user_to_friend_chat_style_preference = (
            profile_memory.user_to_friend_chat_style_preference
        )
    ...
    life_habit_preference = getattr(profile_memory, 'life_habit_preference', None)
    communication_style = getattr(profile_memory, 'communication_style', None)
    occupation = getattr(profile_memory, 'occupation', None)
    gender = getattr(profile_memory, 'gender', None)
    relationship = getattr(profile_memory, 'relationship', None)
    ...
    return {
        ...
        "interests": interests,
        "life_habit_preference": life_habit_preference,
        "communication_style": communication_style,
        "catchphrase": catchphrase,
        "user_to_friend_catchphrase": user_to_friend_catchphrase,
        "user_to_friend_chat_style_preference": user_to_friend_chat_style_preference,
...
def _convert_memcell_to_document(
    ...
    try:
        ...
        # Get owner_user_id from user_id_list (first element is the owner)
        # This allows querying MemCell by owner_user_id, consistent with Episode/Foresight
        primary_user_id = None
        if hasattr(memcell, 'user_id_list') and memcell.user_id_list:
            primary_user_id = memcell.user_id_list[0]
