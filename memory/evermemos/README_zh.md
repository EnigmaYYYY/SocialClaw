...
- 加载 `data/assistant_chat_zh.json`，为每条消息添加 `scene="private"`，并将每条记录流式发送到 `http://localhost:8001/api/v1/memories`。如果您在其他端点托管 API 或想要导入不同的场景，可以更新 `demo/extract_memory.py` 中的 `base_url`、`data_file` 或 `scene` 常量。
...
> ℹ️ `scene` 为必填字段，仅支持 `private` 或 `group`，用于指定记忆提取策略。
...
> - 使用 `private` 用于一对一助手对话
> - 使用 `group` 用于多人群聊讨论
> 
> **注意**：在数据文件中，您可能会看到 `scene` 值为 `work` 或 `company` - 这些是数据格式中的内部场景描述符。命令行参数 `--scene` 使用不同的值（`private`/`group`）来指定应用哪个提取流水线。
